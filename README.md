...
3. タイムラインで成長を観察します。 世代を変更すると、枝の断面と器官を補間して滑らかに伸縮します。補間中は実際に表示している世代も表示します。OSでアニメーションを減らす設定を使っている場合は即時に切り替わります。若い世代は細い幹から徐々に太くなります。再生は安全に生成できる世代で停止します。
...
ビュー左下の「描画品質」で品質を選べます。低は1024pxの影と少ない光源で軽さを優先、中は2048pxの柔らかい影と補助光、高は4096pxの影と背面の補助光を使用します。高DPI画面の描画倍率上限も1 / 1.5 / 2に切り替わります。既定は中で、選択はブラウザーに保存します。「影・霧」と「グリッド」はオン／オフ表示付きの独立スイッチです。「アンチエイリアス」は輪郭を滑らかにする4x MSAAで、再読み込みせず切り替えられ、設定をブラウザーに保存します。
