...
export function setupUI(actions: UIActions) {
  ...
  return {
    sync(params: PlantParams) {
      ...
      element("needle-settings").hidden = !pine && params.growthModel !== "spruce";
      ...
      element("species-help").textContent = ({
        fern: "弓状の葉軸に羽片と小羽片を付けるシダの成長モデルです。",
        pine: "枝先に長い二葉の針葉を付けます。芽ごとに異なる時期に新しい枝が伸びます。",
        oak: "太い主枝が不均等に広がり、丸い裂片の葉を付けるヨーロッパナラです。",
        willow: "上向きの主枝から細長い枝が垂れ下がるシダレヤナギです。",
        spruce: "直立する幹と円錐形の樹冠。短い針葉が小枝の周りに一本ずつ付きます。",
        ginkgo: "上向きの長枝と、扇形の葉が集まる短枝を持つイチョウです。",
        birch: "軽やかな枝と鋸歯のある葉を持つシラカバです。芽ごとに異なる時期に成長します。",
        maple: "広がる枝と深く切れ込む葉を持つモミジです。芽ごとに異なる時期に成長します。",
        sakura: "広がる枝と短い花柄の花を持つサクラです。芽ごとに異なる時期に成長します。",
        lsystem: "文字列のルールから枝分かれを生成します。",
      } satisfies Record<PlantParams["growthModel"], string>)[params.growthModel];
